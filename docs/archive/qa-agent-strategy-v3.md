...
Existing Test Asset Discovery
  ↓
TestReuseCandidate生成
  ↓
DuplicateTestReport生成
  ↓
TestGapReport生成
  ↓
StateModel更新
...
TestabilityReport作成
  ↓
TestArchitectureSpec生成 / 更新
  ↓
TestImpactPlan生成
  ↓
必要な既存テストを選択
  ↓
不足分だけTestDesignSpec生成
  ↓
不足分だけTestAsset生成
  ↓
...
sandbox実行
  ↓
state diff / event / log / screenshot保存
  ↓
Failure Triage
  ↓
QualityAnalyticsSnapshot生成
  ↓
...
| Gate | Block条件 |
|---|---|
| Source grounding gate | P0/P1要求のsource_refsが空 |
| Oracle gate | P0/P1要求にOracleSpecがない |
...
Existing Test Asset Discovery
  ↓
TestReuseCandidate / DuplicateTestReport
  ↓
StateModel generation
  ↓
OracleSpec generation
  ↓
TestImpactPlan
  ↓
不足分だけAPI or Playwright test generation
  ↓
sandbox execution
  ↓
Evidence capture
...
| Dashboard最小版 | static reportまたはinternal pageでRelease Readinessを見られる |
...
必要な既存テスト選択
  ↓
不足分だけAPI/Playwrightテスト生成
  ↓
sandbox実行
  ↓
state diff / logs / screenshots / tool calls保存
  ↓
失敗分類
  ↓
QualityAnalyticsSnapshot生成
