...
- [x] `skill_runner/`(または相当モジュール)で、skill ID を指定して以下が1回の実行で行われる:
...
- [x] LLM呼び出し部分が差し替え可能な境界(interface)になっており、テストでは録画済み応答またはfakeで決定的にテストできる(実LLMなしで `uv run pytest` がpassする)
- [x] 実LLMで最低1回実行し、生成artifactがvalidatorをpassした記録がある(スモークテスト。CIには含めない。実skillは未実装の時点なので `qa-skills/_template` ベースのテスト用ダミーskillで行ってよい)
- [x] 使い方が `skill_runner/README.md` に記載されている(後続skill実装タスクがこのREADMEだけで実装に入れる水準)
- [x] AGENTS.mdのリポジトリ構成マップに `skill_runner/` の行が追加されている
...
- domain / learning-log / decisions へ記録した知見:
  [learning-log 2026-08-02](../../knowledge/learning-log.md)(contract noteの必要性・隔離効果の実測)、
  [ADR-0005 末尾の追記](../../decisions/adr-0005-llm-skill-execution.md)(隔離要件とskill発見機構の非両立)
