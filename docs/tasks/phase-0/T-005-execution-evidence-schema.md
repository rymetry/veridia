...
- [x] `schemas/execution-evidence.schema.json` が存在し、`allOf` でartifact-baseを継承している
- [x] test実行結果・state diff・logsへの参照を表現できる(§6.23のfield構成に準拠)
- [x] 有効サンプルがpassし、不正サンプルがfailするテストがある
